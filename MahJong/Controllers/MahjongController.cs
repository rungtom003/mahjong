...
namespace MahJong.Controllers
{
    public class MahjongController : Controller
    {
        ...
        public async Task<IActionResult> Storedetail(string sid)
        {
            ...
            var timeOpen = await _mahjongDBContext.Store.Where(t => t.SId == sid).FirstOrDefaultAsync();
           
            string time = DateTime.Now.ToString("HH:mm:ss");
            TimeSpan dt = TimeSpan.Parse(time);
            TimeSpan open = TimeSpan.Parse(timeOpen.SOpenTime.ToString());
            if (open > dt)
            {
                ViewBag.checkOpen = true;
                ViewData["store_table"] = tables;
                return View(store);
            }
            else {
                ViewBag.checkOpen = false;
                return View(store);
            }
        ...
        public async Task<IActionResult> Payment()
        {
            if (HttpContext.Session.GetString("username") != null)
            {
                ...
                var myorder = await _mahjongDBContext.BookDetail
                    ...
                    .Include(b => b.T.S)
                    .Include(b => b.B.C)
                    .Where(b => b.B.CId == customerID && b.BdStatus == "ค้างชำระ")
                    .FirstOrDefaultAsync();
                if (myorder != null)
                {
                    ViewBag.myorder = true;
                }
                else
                {
                    ViewBag.myorder = false;
                }
